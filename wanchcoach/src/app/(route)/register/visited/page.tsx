    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      OCRAddPrescriptionController(formData)
        .then((response) => {
          ...
          setDrugValue(ocrItems.map((item: { itemName: string }) => item.itemName));
          setDrugSearching(ocrItems.map(() => false));
          setQuantityValue(ocrItems.map((item: { quantity: number }) => item.quantity));
          setFrequencyValue(ocrItems.map((item: { frequency: number }) => item.frequency));
          setDayValue(ocrItems.map((item: { day: number }) => item.day));
          const updatedDrugs = ocrItems.map((item: PrescriptionDrugData) => ({
            drugId: item.drugId,
